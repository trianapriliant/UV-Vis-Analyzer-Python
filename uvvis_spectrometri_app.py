...
from tkinter import Tk, Label, Entry, Button, filedialog, messagebox, StringVar, Frame, Radiobutton, IntVar, Scale, Checkbutton
    ...
    def plot_graph(show_absorbance=True):
        ...
        for i, df in enumerate(data_frames):
            if show_absorbance:
                ...
                if show_peak_points.get():  # Tampilkan titik puncak jika checkbox dicentang
        ...
        if show_degradasi.get():  # Tampilkan % degradasi jika checkbox dicentang
        ...
        if show_legend.get():  # Tampilkan legend jika checkbox dicentang
...
Radiobutton(input_frame, text="Absorbansi", variable=plot_type, value=1).grid(row=4, column=1, padx=5, pady=0, sticky="w")
Radiobutton(input_frame, text="Transmitansi", variable=plot_type, value=2).grid(row=4, column=2, padx=5, pady=0, sticky="w")
