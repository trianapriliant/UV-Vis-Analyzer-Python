...
def process_data():
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 8))  # Mengubah layout menjadi kolom
    plt.subplots_adjust(hspace=0.4)  # Tambahkan ruang antar subgrafik
    ...
    # Tambahkan Canvas dan Scrollbar untuk grafik
    graph_canvas = Canvas(graph_frame)
    graph_canvas.pack(side="left", fill="both", expand=True)
    
    scrollbar = Scrollbar(graph_frame, orient="vertical", command=graph_canvas.yview)
    scrollbar.pack(side="right", fill="y")
    
    scrollable_frame = Frame(graph_canvas)
    ...
    # Tampilkan grafik dalam scrollable frame
    canvas = FigureCanvasTkAgg(fig, master=scrollable_frame)
    canvas.draw()
    canvas.get_tk_widget().pack()
    
    # Tambahkan toolbar interaktif
    toolbar = NavigationToolbar2Tk(canvas, scrollable_frame)
    toolbar.update()
    toolbar.pack()
...
Entry(root, textvariable=folder_path_var, width=50).grid(row=0, column=1, padx=10, pady=5, sticky="we")
...
Entry(root, textvariable=sample_names_var, width=50).grid(row=1, column=1, padx=10, pady=5, sticky="we")

# Input untuk judul grafik
Label(root, text="Graph Title:").grid(row=2, column=0, padx=10, pady=5, sticky="w")
graph_title_var = StringVar()
Entry(root, textvariable=graph_title_var, width=50).grid(row=2, column=1, padx=10, pady=5, sticky="we")

# Process button
Button(root, text="Process Data", command=process_data).grid(row=3, column=0, columnspan=3, pady=10)

# Frame untuk grafik dengan Scrollbar
graph_frame = Frame(root)
graph_frame.grid(row=4, column=0, columnspan=3, pady=10, sticky="nsew")

# Membuat scrollbar
scrollbar = Scrollbar(graph_frame, orient="vertical")
scrollbar.pack(side="right", fill="y")

# Membuat canvas di dalam graph_frame
canvas_widget = Canvas(graph_frame, yscrollcommand=scrollbar.set)
canvas_widget.pack(side="left", fill="both", expand=True)

# Konfigurasi scrollbar agar berfungsi dengan canvas
scrollbar.config(command=canvas_widget.yview)

# Membuat frame di dalam canvas untuk memasukkan grafik
canvas_content = Frame(canvas_widget)
canvas_widget.create_window((0, 0), window=canvas_content, anchor="nw")

# Fungsi untuk mengatur scroll region
def configure_scroll_region(event):
    canvas_widget.config(scrollregion=canvas_widget.bbox("all"))

# Bind event ketika isi canvas berubah
canvas_content.bind("<Configure>", configure_scroll_region)
